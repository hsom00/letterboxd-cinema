...
const STEP_LABELS = { 0: "Let's go", 1: 'Continue', 2: 'Continue', 3: 'Continue', 4: 'Set up my cinema', 6: 'Enter' };
function showStep(n) {
  wizard.step = n;
  ...
  $('#setup-back').hidden = n === 0 || n >= 5;
  $('#setup-next').hidden = n === 5;
  $('#setup-next').textContent = STEP_LABELS[n] || 'Continue';
...
async function loadSources() {
  ...
  try {
    ...
    box.innerHTML = d.sources?.length ? d.sources.map(s => `<label><input type="checkbox" name="indexers" value="${esc(s.id)}"><span><span class="n">${esc(s.name)}</span><span class="d">${esc(s.description)}</span></span></label>`).join('')
      : `<span class="label">None available yet — you can add sources later in Settings.</span>`;
    box.dataset.loaded = '1';
    box.addEventListener('change', () => { const n = box.querySelectorAll('input:checked').length; $('#sources-count').textContent = n ? `${n} chosen` : ''; });
  } catch { box.innerHTML = `<span class="label">Could not load the list — you can add sources later in Settings.</span>`; }
}
const WAITING = ['Waking the projection booth…', 'Creating your account…', 'Building the library…', 'Teaching Radarr where things go…', 'Connecting the sources…', 'Nearly there…'];
async function runSetup() {
  showStep(5);
  const log = $('#setup-log'); let i = 0;
  log.innerHTML = `<li class="busy">${WAITING[0]}</li>`;
  const ticker = setInterval(() => { i = Math.min(i + 1, WAITING.length - 1); log.firstElementChild.textContent = WAITING[i]; }, 9000);
  try {
    ...
    log.innerHTML = (d.log || []).map(l => { const ok = / ok$/.test(l) || /every 15 minutes/.test(l) || /started$/.test(l); return `<li class="${ok ? 'ok' : 'bad'}">${esc(l.replace(/ ok$/, ''))}</li>`; }).join('');
    const bad = (d.log || []).filter(l => !/ ok$/.test(l) && /source/.test(l)).length;
    if (bad) $('#ready-copy').textContent = `Sign in with the account you just made. ${bad} of the sources you chose ${bad === 1 ? "wasn't" : "weren't"} reachable just now — they're saved but switched off, and you can try them again in Settings. Films from your watchlist will start arriving shortly.`;
    setTimeout(() => showStep(6), 1200);
  } catch (e) { clearInterval(ticker); log.innerHTML = `<li class="bad">Something went wrong: ${esc(e.message)}</li>`; $('#setup-back').hidden = false; }
}
$('#setup-form').addEventListener('submit', (e) => {
  e.preventDefault();
  const f = new FormData(e.target); const err = (m) => { $('#setup-err').textContent = m; $('#setup-err').hidden = false; };
  if (wizard.step === 0) return showStep(1);
  if (wizard.step === 1) { const v = (f.get('name') || '').trim(); if (!v) return err('Give it a name — you can always change it.'); wizard.data.name = v; return showStep(2); }
  if (wizard.step === 2) {
    const u = (f.get('admin_user') || '').trim(), p = f.get('admin_password') || '';
    if (!u) return err('Choose a username.'); if (/\s/.test(u)) return err('No spaces in the username, please.');
    if (p.length < 8) return err('Use a password of at least 8 characters — this account can be reached from the internet.');
    wizard.data.admin_user = u; wizard.data.admin_password = p; return showStep(3);
  }
  if (wizard.step === 3) { wizard.data.letterboxd_user = (f.get('letterboxd_user') || '').trim().replace(/^.*letterboxd\.com\//, '').replace(/\/.*$/, '').replace(/^@/, ''); return showStep(4); }
  if (wizard.step === 4) { wizard.data.indexers = f.getAll('indexers'); return runSetup(); }
  if (wizard.step === 6) { $('#setup').hidden = true; document.body.dataset.view = 'login'; $('#login').hidden = false; $('#login-form').user.value = wizard.data.admin_user; $('#login-form').pass.focus(); loadConfig(); }
});
$('#setup-back').onclick = () => showStep(Math.max(0, wizard.step - 1));
$('#pw-toggle').onclick = () => { const i = $('#setup-form').admin_password; i.type = i.type === 'password' ? 'text' : 'password'; $('#pw-toggle').textContent = i.type === 'password' ? 'Show' : 'Hide'; };

async function start() {
  document.documentElement.style.setProperty('--grain', makeGrain());
  await loadConfig();
  if (config.setup) { document.body.dataset.view = 'setup'; $('#setup').hidden = false; showStep(0); return; }
